...
![SKILL tab: 技能包 / 散装技能 / 预设层开关](docs/images/prompt.png)

![MCP tab: Server 卡片、分层遮蔽与工具级启停](docs/images/tool.png)
...
- **拖拽**：把「本系统全部提示词」池里的行拖进列表 = 加入当前阶段（带上池里原文）；把行拖回池 = 从当前阶段移除（注入进来的撤销注入，原生段转为屏蔽）；把行拖到头部某个**阶段按钮**上 = 复制到那个阶段。拖动中有插入位置提示（行上下的蓝色线），投放区高亮。
