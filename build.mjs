/**
 * dsh-prompt-customizer — build script.
 *
...
// 可选：指向一份 DSH 检出，用于在没装 esbuild 时借用它 pnpm store 里的副本。
// 缺省为空 = 只认本仓库 node_modules（正常安装路径）。
const DSH_CHECKOUT = process.env.DSH_CHECKOUT ?? ''
const PLUGIN_ID = 'dsh-prompt-customizer'
...
function loadEsbuild() {
  ...
  throw new Error(
    'dsh-prompt-customizer: cannot find esbuild.\n'
...
const clientBundle = {
  ...
  // 浏览器半身落在 client/：package.json 的 exports.\"./client\" 指向它。
 ...
 * that DSH actually ships runtime JS for. `@deepseek-ai/cordis` does. Anything
 * else under `@deepseek-ai/` must be vendored into `vendor/` instead — see
 * vendor/README.md.
 *
...
const hostBundle = {
  ...
  sourcemap: true,
  logLevel: 'info',
  // node:* builtins stay external. js-yaml is bundled in on purpose: the
  ...
  // cannot also hold a private js-yaml copy.
  //
  ...
  external: [],
  // js-yaml is inlined (see above) and is CJS: it calls bare `require('fs')`.
...
function assertHostExternals(outfile) {
  ...
  if (violations.length > 0) {
    throw new Error(
      'dsh-prompt-customizer: host bundle imports packages that an installed plugin cannot resolve.\n'
...
console.log('[dsh-prompt-customizer] built lib/index.js + client/client.js')
console.log(`[dsh-prompt-customizer] host runtime imports: ${hostExternals.length === 0 ? '(none)' : hostExternals.join(', ')}`)
