/**
 * dsh-prompt-customizer — MCP 单工具级启停回归测试（两层账本 + 按「行」记账）。
 *
...
const sandbox = mkdtempSync(join(tmpdir(), "dsh-prompt-customizer-tool-"))
...
const ledgerFile = join(process.env.DSH_HOME, "mcp", "dsh-prompt-customizer", "tool-disable.json")
mkdirSync(join(process.env.DSH_HOME, "mcp", "dsh-prompt-customizer"), { recursive: true })
...
const maskLedgerFile = join(process.env.DSH_HOME, "mcp", "dsh-prompt-customizer", "preset-masks.json")
