...
export function apply(ctx, entry = {}) {
  ...
  ctx.on('system-prompt/assemble', async (assembly, context, next) => {
    ...
    for (const item of injectList) {
      // Skip sections that are blocked — they were already filtered out above.
      ...
      if (index >= 0) {
        ...
        // Preserve any replacement text set above; only use item.text when no
        // replacement exists and the user actually provided inject text.
    ...
    sections.sort((a, b) => {
      // If injectList is non-empty, use it as the authoritative order.
      // Sections in injectList come first (in injectList order), then remaining
      // sections sorted by their original order.
