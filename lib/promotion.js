 ...
 * / reload 重建出相同的阶段。
 *
 * 会话事件读取：宿主 0.1.2-alpha.4 起移除了 `Session.events`（插件原先的读法在
 * 该版本之后静默拿到空数组），替代入口是 `session.snapshotEvents()`。
 * TODO: 该同步历史读在宿主 0.1.6 已被标记 deprecated（见 .agents/notes/implemented/
 * architecture/2026-09-09-deprecate-synchronous-session-event-reads.md，长期方向是
 * 投影状态 + 增量维护）；等宿主提供阶段投影、或本插件自注册 projection unit 后，
 * 把下面的冷扫描换成投影读取。
 */
